...
class RoadLines:
    ...
    def find_lane_pixels_binary(self, binary_warped):
        def moving_avg(x, n):
    ...
    def fit_polynomial(self, binary_warped):
        if not self.detected:
            leftx, lefty, rightx, righty = self.find_lane_pixels_binary(binary_warped)
        else:
        ...
        #Calculate points from polynomial
...
def get_processed_image(img):
    ...
    plt.show()
    image_with_lines = road_lines.fit_polynomial(binary_warped)
...
def test_image():
    ...
    for fname in images:
        ...
        cv2.imwrite("/home/homisiu/workspace/ad_udacity/CarND-Advanced-Lane-Lines/output_images/test_images/" + os.path.basename(fname),
...
def test_video():
    ...
    white_output = 'output_videos/' + name
    ## To speed up the testing process you may want to try your pipeline on a shorter subclip of the video
    ## To do so add .subclip(start_second,end_second) to the end of the line below
    ## Where start_second and end_second are integer values representing the start and end of the subclip
    ## You may also uncomment the following line for a subclip of the first 5 seconds
    clip1 = VideoFileClip(name)
    white_clip = clip1.fl_image(get_processed_image)  # NOTE: this function expects color images!!
